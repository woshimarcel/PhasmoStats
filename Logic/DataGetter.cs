...
public static class DataGetter
{
    ...
    public static Dictionary<string, (int seen, int died, double ratio, double percentage)> GetGhosts(Dictionary<string, object> data)
    {
        ...
        var stats = new Dictionary<string, (int seen, int died, double ratio, double percentage)>();
        ...
        foreach (string ghostType in ghostTypes)
        {
            ...
            double percentage = totalCases > 0 && hasSeen ? (double)seen / totalCases : 0;
        ...
        int totalGhosts = stats.Values.Sum(i => i.seen);
        if (totalCases != totalGhosts)
        {
            int seen = totalCases - totalGhosts;
            int died = GetInt(data, SaveKeys.DEATHS) - stats.Values.Sum(i => i.died);
            double ratio = seen > 0 && died > 0 ? (double)died / seen : 0;
            double percentage = totalCases > 0 && seen > 0 ? (double)seen / totalCases : 0;
            stats["UNKNOWN"] = (seen, died, ratio, percentage);
        }
    ...
    public static Dictionary<string, (int visits, double percentage)> GetMaps(Dictionary<string, object> data)
    {
        ...
        Dictionary<string, (int visits, double percentage)> maps = [];
        int totalCases = GetTotalCases();
        bool hasPlayed = totalCases > 0;
        ...
        foreach (var kv in tempMaps)
        {
            ...
            double percentage = hasPlayed && hasVisited ? (double)kv.Value / totalCases : 0;
    ...
    private static void GetUnknownAmount()
    {
